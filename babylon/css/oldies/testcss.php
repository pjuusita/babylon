...
echo "}\n";*/
			...
			foreach($registry->cssDefinitions as $cssDefinitions => $cssDefinition) {
				if ($cssDefinition->cssclassID == $cssClass->cssclassID) {
					if ($cssDefinition->themeitemID != 0) {
						echo "\t" . $cssDefinition->propertyname . ":" . $registry->themeItemValues[$cssDefinition->themeitemID] . ";\n";
					} else {
	...
	function loadCSS($cssfileID, $themeID) {
		...
		$registry->cssClasses = Table::load('system_cssclasses', " WHERE CssfilesID='" . $cssfileID . "'");
	...
	function main() {
		$registry = loadCSS(1,43);
		constructCSS($registry,1);
	}
