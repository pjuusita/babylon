<?php
	...
	$sql = "SELECT * FROM worder_words WHERE LanguageID=2 AND WordclassID=" . $wordclassID . " AND GrammarID=1";
	...
	$sql = "SELECT * FROM worder_wordforms WHERE LanguageID=2 AND WordclassID=" . $wordclassID . " AND GrammarID=1";
	echo "<br>sql - " . $sql;
	...
	foreach($words as $wordID => $lemma) {
		...
		if ($singularpresent == true) {
			...
			$found = false;
			$sql = "INSERT INTO worder_wordforms (Wordform, WordID, Features, Grammatical, SystemID, GrammarID, LanguageID, WordclassID, Defaultform) VALUES ('" . $lemma . "',"  . $wordID . ", '259',1, 5, 1, 2, " . $wordclassID . ", 1)";
			echo "<br>Sql - " . $sql;
