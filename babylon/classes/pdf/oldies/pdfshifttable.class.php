...
class PDFShiftTable extends PDFComponent {
	...
	public function __construct($left,$top,$width,$data,$base_pdf,$limiter=null) {
		$this->left     = $left;
		$this->top      = $top;	
		$this->data     = $data;
		$this->width	= $width;
	...
	private function drawTableHeader($left,$top) {
		...
		foreach($columns as $index => $column) {
			...
			$this->drawHeaderContent($item_left+$header_margin,$item_top,$column);
	...
	private function drawHeaderContent($left,$top,$column) {
	
		$pdf = $this->FPDF;
	
		$value      = $column->column_name;
		
		$font_name  = $column->header_font_name;
		$font_style = $column->header_font_style;
		$font_size  = $column->header_font_size;
		$font_color = $column->header_font_color;
	...
	private function drawTableRows() {
		...
		foreach($data as $index => $item) {
			...
			$partition	= $item->$limiter;
			
			if (($partition!=$oldPartition)){
				
				//if (!$firstPartition) $this->drawSums($y);
			...
			//$this->drawRowBackground($x,$y,$backgroundRowNumber);
			...
			foreach($columns as $index => $column) {
				...
				$value = $column->format($item->$variable_name);
				
				if (($limiter==$variable_name) && ($drawLimiter)) {
					$this->drawTableContent($x,$y,$value);
					$drawLimiter = false;
				}
				
				if ($limiter!=$variable_name) $this->drawTableContent($x,$y,$value);
				
				if ($column->type=='sumcolumn') $column->sum($value);
	...
	private function drawSums($y) {
		...
		foreach($columns as $index=>$column) {
			...
			if ($column->type=='sumcolumn') {
				
				$value	= $column->getCurrentSum();
				$column->setCurrentSum(0);
