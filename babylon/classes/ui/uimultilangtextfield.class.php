...
class UIMultiLangTextField extends UIField {
	...
	public function createShowDiv($languages,$items, $visible) {
		...
		echo "		<tr>";
		echo "			<td  class=field-text style='width:26%;vertical-align:top;'>";
		echo "				" . $this->title;
		echo "			</td>";
		echo "			<td style='width:50%'>";
		$currentlangID = getSystemLang();
		if (isset($items[$currentlangID])) {
			echo "".$items[$currentlangID]."";
		} else {
			echo "Ei asetettu";
		}
	...
	public function createEditDiv($languages, $items, $visible) {
		...
		echo "	<table style='width:100%'>																				";
		echo "		<tr>																								";
		echo "			<td class=field-text style='width:26%;vertical-align:top;padding-top:7px;'>";
		echo "				".$title;
		echo "			</td>																							";
		echo "			<td style='width:50%'>																			";
		echo "				<table>";
	...
	private function createScripts($languages) {
		...
		echo "			console.log('setfieldvalue aaa - '+value);";
		
		// tyhjennetään ensin kaikki kentät
		foreach($languages as $index => $language) {
			echo "		$('#language_edit_".$this->getID()."_" . $language->languageID . "').val('');";
		}
		
		echo "			var items = value.split('[');";
		echo "			for(var key in items) {";
		echo "				console.log('key: '+key+', item:'+items[key]);";
		echo "				pair = items[key].split(']');";
		echo "				console.log('lang-'+pair[0]+'..'+pair[1]);";
		echo "				$('#language_edit_".$this->getID()."_'+pair[0]).val(pair[1]);";
		echo "			}";
		...
		echo "		var value = '';";
		foreach($languages as $index => $language) {
			echo "	value = $('#language_edit_".$this->getID()."_" . $language->languageID . "').val();";
			echo "	if (value != '') {";
			echo "		params = params + '[" . $language->languageID . "]' + value;";
			echo "	}";
		}
		echo "		return params;";
		echo "	}";
	...
	public function show($data = null) {
		...
		if ($data == null) {
			...
			$variable = $this->dataVariable;
			//echo "<br>Data - " . $variable;
			//echo "<br>Data2 - " . $data->$variable;
			
			$var = $this->dataVariable;
			if ($data == null) {
				$value = "";
			} else {
				if (is_array($data)) {
			...
			$items = parseMultilangArray($data->$variable);
		}
