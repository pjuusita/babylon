...
class UIFloatField extends UIField {
	...
	*/
	...
	private function createScripts() {
		...
		echo "																											";
		echo "			if ((!valid) || (valid=='') alert('Invalid float value');										";	
		echo "	}																										";
		echo "																											";
		echo "</script>																									";
		...
		/*
		 * Kopioitu vanhasta floadfieldista, taalla on muodon tarkistukseen tarpeellista kalustoa
		 * 
		 * 
		  public function outputContent($width) {
		
			echo "	<input id=editfield-".$this->getID()."-".$this->getID()." type='text' onkeyup='editfieldchanged(".$this->getID().",".$this->getID().")' onfocusin='editfieldactivated(".$this->getID().",".$this->getID().")' onfocusout='editfielddeactivated(".$this->getID().",".$this->getID().")' class=sectioninputfield style='display:block;' type='text' name=".$this->parametername." value='".$this->value."'>";
			
			echo "	<script>";
			echo " 		function checkvalues_".$this->getID()."_".$this->getID()."(event) {";
			echo "			var value=$('#editfield-".$this->getID()."-".$this->getID()."').val();";
			echo "			if (value != '')  {";
			echo "			if (!/^-?[0-9]+[.|,]?[0-9]*$/.test(value)) {";
			echo "				seterror('".$this->getID()."','".$this->getID()."','Syata numero');";
			echo "				return false;";
			echo "			}";
			echo "			}";
			if ((isset ($params['notempty'])) && ($params['notempty'] == true)) {
				echo "		if (value == '') {";
				echo "			seterror('".$this->getID()."','".$this->getID()."','Kentta ei saa olla tyhja');";
				echo "			return false;";
				echo "		}";
			}
			if (isset ($params['maxvalue'])) {
				echo "		if ((".$params['maxvalue']." < value) && (value != '')) {";
				echo "			seterror('".$this->getID()."','".$this->getID()."','Maksimi arvo on ".$params['maxvalue']."');";
				echo "			return false;";
				echo "		}";
			}
			if (isset ($params['minvalue'])) {
				echo "		if ((".$params['minvalue']." > value) && (value != '')) {";
				echo "			seterror('".$this->getID()."','".$this->getID()."','Minimi arvo on ".$params['minvalue']."');";
				echo "			return false;";
				echo "		}";
			}
			echo "			hideerror(".$this->getID().",".$this->getID().");";
			echo "			return true;";
			echo "		};";
			echo "	</script>";
		}
