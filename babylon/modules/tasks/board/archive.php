...
$projectfilter->addSelectFilter($this->registry->projectID, $this->registry->projects, "tasks/board/archive", "", "projectID", "name");
$projectfilter->setEmptySelect(false);


$labelfilter = new UIFilterBox();
$labelfilter->addSelectFilter($this->registry->labelID, $this->registry->labels, "tasks/board/archive", "", "labelID", "name");
...
echo "<h1>Arkisto - " . $registry->project->name . "</h1>";
echo "		</td>";
...
echo "<table style='width:700px;'>";
echo "	<tr>";
echo "		<td style='width:100%;vertical-align:bottom'>";
echo "<h1>Arkisto - " . $registry->project->name . "</h1>";
echo "		</td>";
echo "	</tr>";
echo "</table>";
...
foreach($this->registry->tasks as $index => $task) {
	...
	echo "				<a class=a-taskboard href='". getUrl('tasks/board/showtask') . "&tasksource=archive&id=" . $task->taskID . "'>";
...
if (count($this->registry->tasks) == 0) {
	echo "<br>Ei yhtään arkistoitua tehtävää";
}


echo "<script>";
echo "	function taskrowclicked(taskID) {";
echo "			window.location='".getUrl('tasks/board/showtask')."&tasksource=archive&id='+taskID;";
echo "	}";
echo "</script>";
