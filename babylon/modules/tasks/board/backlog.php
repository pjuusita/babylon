...
$boardfilter->addSelectFilter($this->registry->projectID, $this->registry->projects, "tasks/board/backlog", "", "projectID", "name");
$boardfilter->setEmptySelect(false);


$labelfilter = new UIFilterBox();
$labelfilter->addSelectFilter($this->registry->labelID, $this->registry->labels, "tasks/board/backlog", "", "labelID", "name");
...
echo "	<tr>";
echo "		<td rowspan=3 style='width:60%;vertical-align:bottom'>";
echo "			<h1>Backlog - " . $registry->project->name . "</h1>";
echo "		</td>";
...
echo "	</tr>";
echo "	<tr>";
echo "			<td class=pagetitle style='font-size:24px;font-weight:bold;text-align:right;vertical-align:bottom;padding-top:20px;'>";
...
foreach($this->registry->tasks as $index => $task) {
	...
	echo "				<a class=a-taskboard href='". getUrl('tasks/board/showtask') . "&tasksource=backlog&id=" . $task->taskID . "'>";
	echo "					<div style='width:100%;height:100%;'>";
	echo "						<div style='width:360px;overflow:hidden;height:18px;padding-top:3px;padding-bottom:3px;'>";
	echo "						" . $task->name . "";
	echo "						</div>";
	echo "					</div>";
	echo "				</a>";
...
if (count($this->registry->tasks) == 0) {
	echo "<br>Ei yhtään tehtävää";
}


echo "<script>";
echo "	function taskrowclicked(taskID) {";
echo "			window.location='".getUrl('tasks/board/showtask')."&tasksource=backlog&id='+taskID;";
echo "	}";
echo "</script>";
