 ...
 * 
 * Periaatteessa jos järjestelmässä on erittäin vähän käyttäjiä/käyttäjäryhmiä jne. niin nämä voisi sijaita kaikki pelkästään
 * yhdellä sivulla 'asetukset'. Nythän alkaa olla jo aika paljon erilaisia asetuksia. Mutta mennään nyt näin kun tämä on 
 * helpompi toteuttaa.
 *
 *
 */
class SettingsController extends AbstractController {
	...
	public function showsettingsAction() {
		...
		foreach($settings as $index => $row) {
			...
			if ($row->name == 'system_settings_savedir') {
				$settingsrow->savedir = $row->value;
				//echo "<br>found - " . $row->value;
			}
		}
		...
		*/
		...
		/*
		$modulefile = $this->path . 'modules' . DIRECTORY_SEPARATOR . $this->registry->modulename . DIRECTORY_SEPARATOR . $this->registry->modulename . ".module.php";
