...
echo "		<td style='padding-left:10px;padding-top:4px;max-width:300;width:300;text-align:right;'>";
$button3->show();
$button1->show();
$button2->show();
$settingsbutton->show();
...
foreach($this->registry->activelanguages as $index => $languageID) {
	...
	if ($this->registry->rulesetID == 0) {
		echo "			var loc = '" . getUrl("worder/sentences/translatesentenceJSON") . "&id='+sentenceID+'&targetlanguageID='+languageID+'&rulesetID=" . $this->registry->set->rulesetID . "';";
	} else {
		echo "			var loc = '" . getUrl("worder/sentences/translatesentenceJSON") . "&id='+sentenceID+'&targetlanguageID='+languageID+'&rulesetID=" . $this->registry->rulesetID . "';";
	}
...
foreach($this->registry->activelanguages as $index => $languageID) {
	...
	echo "			event.stopPropagation();";
	if ($this->registry->rulesetID == 0) {
		echo "			var loc = '" . getUrl("worder/sentences/translatesentenceJSON") . "&comments=1&id='+sentenceID+'&targetlanguageID='+languageID+'&rulesetID=" . $this->registry->set->rulesetID . "';";
	} else {
		echo "			var loc = '" . getUrl("worder/sentences/translatesentenceJSON") . "&comments=1&id='+sentenceID+'&targetlanguageID='+languageID+'&rulesetID=" . $this->registry->rulesetID . "';";
	}
...
echo "<script>";
echo "		function chekbuttonpressed(event, sentenceID, rowNumber) {";
echo "			event.stopPropagation();";
echo "			var loc = '" . getUrl("worder/sentences/checksentence") . "&comments=1&id='+sentenceID;";
echo "			console.log(loc);";
echo "			$.getJSON(loc,'',function(data) {";
