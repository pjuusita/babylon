...
echo "		opennewtab('" . getPdfUrl("accounting/balancesheet/balancesheetpdf")."&selectionID=" . $this->registry->selectionID . "&periodID=" . $this->registry->periodID . "');";
echo "	});";
echo "</script>";
...
$tabsection = new UITabSection("","920px");

$tabIndex = $tabsection->addTab("Avaus", "accounting/balancesheet/showbalancesheet&selectionID=-1");
if ($this->registry->selectionID == -1) {
...
function generateLevelThree($account, $accounts, $entries, $allaccounts) {
	...
	foreach($childs as $index => $childaccount) {
		...
		if (($childaccount->accounttypeID == 2) || ($childaccount->accounttypeID == 3)) {
			$sum = $sum * -1;
			$startamount = $tempaccount->startamount * -1;
		} else {
		...
		if ($entrycount > 0) {
			...
			$tempaccount = $allaccounts[$childaccount->accountID];
			echo "		<td style='width:150px;font-size:20px;text-align:right;'>" . number_format($startamount,2,","," ") . " €</td>";
			echo "		<td style='width:150px;font-size:20px;text-align:right;'>" . number_format($sum,2,","," ") . " €</td>";
...
function generateLevelFour($account, $accounts, $entries, $allaccounts) {
	...
	foreach($childs as $index => $childaccount) {
		...
		if (($tempaccount->accounttype == 2) || ($tempaccount->accounttype == 3)) {
			echo "		<td style='width:150px;font-size:20px;text-align:right;'>" . number_format($startamount,2,","," ") . " €</td>";
			echo "		<td style='width:150px;font-size:20px;text-align:right;'>" . number_format(((($sum))),2,","," ") . " €</td>";
			//echo "		<td style='width:150px;font-size:20px;text-align:right;'>b1 " . number_format(((($tempaccount->startamount + $selectionamount))),2,","," ") . " €</td>";
		} else {
			echo "		<td style='width:150px;font-size:20px;text-align:right;'>" . number_format($startamount,2,","," ") . " €</td>";
			echo "		<td style='width:150px;font-size:20px;text-align:right;'>" . number_format(((($sum))),2,","," ") . " €</td>";
			//echo "		<td style='width:150px;font-size:20px;text-align:right;'>d1 " . number_format(((($tempaccount->startamount + $selectionamount))),2,","," ") . " €</td>";
		}
		$selectionamount = $sum;
